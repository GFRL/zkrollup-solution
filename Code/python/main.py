...
def compute3(type=0):
    if type == 0 :#足够多钱
        ...
        #if (E2 < 0):   
            #return
        print("C="+str(C)+" and X1="+str(X1)+" and x2="+str(X2)+" and x11="+str(X11)+" and x22="+str(X22)+" and E1="+str(E1)+" and E2="+str(E2))
        LX1.append(X1)
        LX2.append(X2)
        LX11.append(X11)
        LX22.append(X22)
...
def compute(type=0):
    if type == 0 :#足够多钱
        X1 = solve_second((N+2)*(N+2)+(2*N+3)*S, 2 * C * (N + 2) - (N + 1) * M+2*S*C,  C*C-C*M)
        X2 = solve_second((N+1)*(N+1)+(2*N+1)*S, 2 * C * (N + 1) - (N) * M+2*S*C,  C*C-C*M)
        E1 = (M - S*X1) * X1 / ((N + 2) * X1 + C) - X1#平衡点运算的时候进行了变形，X_i实际上是X_i-b;
        E2 = (M - S*X2) * X2 / ((N + 1) * X2 + C) - X2
        #if (E2 < 0):
         #   return
        #print("M=%.0lf,C=%.0lf(%.0lf),b=%.0lf\n", M, C, C / C_average, b)
        if(E1<=0):
        ...
        X1 = C_suppose
        X2 = 2 * C_suppose
        X11 = solve_second((N+2)*(N+2)+(2*N+3)*S, 2 * C * (N + 2) - (N + 1) * M+2*S*C,  C*C-C*M)
        X22 = solve_second((N+1)*(N+1)+(2*N+1)*S, 2 * C * (N + 1) - (N) * M+2*S*C,  C*C-C*M)
        if (X1 > X11) :
            X1 = X11 
          #  return
        else:
            X11 = solve_second(N*N+(2*N-1)*S, 2 * (C+2*X1) * (N) - (N - 1) * M+2*S*(C+2*X1), 
                                (C+2*X1)*(C+2*X1)-(C+2*X1)*M)
        if (X2 > X22) :
            X2 = X22 
           # return
        else:
            X22= solve_second(N*N+(2*N-1)*S, 2 * (C+X2) * (N) - (N - 1) * M+2*S*(C+X2), 
                                (C+X2)*(C+X2)-(C+X2)*M)
        E1 = (M  - S * X1)*X1 / (N * X11 + C + 2 * X1) - X1 
        E2 = (M  - S* X2) *X2/ (N * X22 + C + X2) - X2
        if(E1<=0):
            X1=0
            E1 = 0
            X11=0
...
plt.figure(1)
plt.savefig("./new example4_detail tempetation.png")

plt.figure(2)
plt.savefig("./new example4_detail benifit.png")

plt.figure(3)
plt.savefig("./new example4_detail benifit percent.png")


# plt.figure(1)
# plt.savefig("./new example4_simple tempetation.png")

# plt.figure(2)
# plt.savefig("./new example4_simple benifit.png")

# plt.figure(3)
# plt.savefig("./new example4_simple benifit percent.png")
